...
def filtre_uygula(img, kernel, islem):
    ...
    padded_img = np.pad(img, ((bol_m, bol_m), (bol_n, bol_n)),
    ...
    for satir in range(bol_m, M-bol_m):
        for sutun in range(bol_n, N-bol_n):
            giris_pencere = padded_img[satir-bol_m: satir +
                                       bol_m + 1, sutun-bol_n:sutun + bol_n+1]
            out_img[satir, sutun] = islem(giris_pencere, kernel)
    return out_img.astype(np.uint8)


def korelasyon(giris, kernel):
    def yapilacak_islem(giris_pncere, kernel): return (
        giris_pncere*kernel).sum()
...
def medyan_filtre(giris, m, n):
    ...
    def yapilacak_islem(giris_pncere, kernel): return np.median(giris_pncere)
...
hstacked4 = np.hstack((cv2.resize(img, boyut),
                       cv2.resize(
                           gauss_kernel_konvolusyon_resimleri[0], boyut),
                       cv2.resize(
                           gauss_kernel_konvolusyon_resimleri[1], boyut),
                       cv2.resize(
                           gauss_kernel_konvolusyon_resimleri[2], boyut),
                       ))
